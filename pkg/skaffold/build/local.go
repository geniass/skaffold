...
func (l *LocalBuilder) Build(ctx context.Context, out io.Writer, tagger tag.Tagger, artifacts []*config.Artifact) (*BuildResult, error) {
	...
	for _, artifact := range artifacts {
		if artifact.DockerArtifact != nil && artifact.DockerArtifact.BaseImages != nil {
			// its a Docker artifact and it depends on 1 or more base images
			logrus.Debugf("Image has 1 or more base image dependencies")
			if artifact.DockerArtifact.BuildArgs == nil {
				artifact.DockerArtifact.BuildArgs = map[string]*string{}
			}
			for _, build := range res.Builds {
				for _, baseImage := range artifact.DockerArtifact.BaseImages {
					if build.ImageName == baseImage.ImageName {
						logrus.Debugf("Found matching base image: %s", build.ImageName)
						if build.Tag == "" {
							return nil, errors.Errorf("getting base image build result: Tag is empty: baseImage=%s", build.ImageName)
						}

						// found a match, so set its BuildArg and keep looking for more
						if _, ok := artifact.DockerArtifact.BuildArgs[baseImage.BuildArg]; ok {
							logrus.Warningf("BuildArg %s will be overwritten because a base image is present", baseImage.BuildArg)
						}
						artifact.DockerArtifact.BuildArgs[baseImage.BuildArg] = &build.Tag
					}
				}
			}
		}
